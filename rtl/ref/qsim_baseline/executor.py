...
class Executor:
    ...
    def _matrix(self, d: dict):
        ...
        for b in range(batch):
            ...
            if dequant:
                C = self._gemm_dequant(A, B, M, N, K, G, scale_dtype, scale_base)
    ...
    def _gemm_dequant(self, A: np.ndarray, B: np.ndarray, M, N, K, G,
                      scale_dtype, scale_base) -> np.ndarray:
        """W8A8 dequant: INT32 per-128-K-group accumulation -> fp32 scale ->
        fp-domain accumulation across groups (02 §6, 04 §1.5)."""
