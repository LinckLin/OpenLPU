...
`ifdef POWER_PINS
module kh4096x64 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, RET1N);
`else
module kh4096x64 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, RET1N);
`endif
 ...
 wire  CLK_;
  ...
  assign RET1N_ = RET1N;
   ...
   `ifdef ARM_FAULT_MODELING
     kh4096x64_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module kh4096x64_error_injection (Q_out, Q_in, CLK, A, CEN, WEN);
   output [63:0] Q_out;
   input [63:0] Q_in;
   input CLK;
   input [11:0] A;
