...
`ifdef POWER_PINS
module ang4096x64 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, RET1N);
`else
module ang4096x64 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, RET1N);
`endif
 ...
 wire  CLK_;
  ...
  assign RET1N_ = RET1N;
   ...
   `ifdef ARM_FAULT_MODELING
     ang4096x64_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
  ...
  reg NOT_A3, NOT_A2, NOT_A1, NOT_A0, NOT_D63, NOT_D62, NOT_D61, NOT_D60, NOT_D59;
 ...
 wire  CLK_;
  ...
  buf B141(D_[62], D[62]);
  buf B142(D_[63], D[63]);
  buf B143(EMA_[0], EMA[0]);
  buf B144(EMA_[1], EMA[1]);
  buf B145(EMA_[2], EMA[2]);
  buf B146(EMAW_[0], EMAW[0]);
  buf B147(EMAW_[1], EMAW[1]);
  buf B148(EMAS_, EMAS);
  buf B149(RET1N_, RET1N);
...
`endif
`timescale 1ns/1ps
module ang4096x64_error_injection (Q_out, Q_in, CLK, A, CEN, WEN);
   output [63:0] Q_out;
   input [63:0] Q_in;
   input CLK;
   input [11:0] A;
